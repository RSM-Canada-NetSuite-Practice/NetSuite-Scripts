...
define(['N/file', 'N/search', 'N/record', 'N/currency'], function(file, search, record, currency) {
  ...
  function getInputData() {
    ...
    var salesorderSearchObj = search.create({
      ...
      filters: [
        ...
        "AND",
        ["formulanumeric: CASE WHEN {custrecord_celigo_amzio_set_parent_tran.custrecord_celigo_amzio_set_posted_date} <> {applyingtransaction.trandate} THEN 1 ELSE 0 END", "equalto", "1"],
        "AND",
  ...
  function map(context) {
    ...
    var invoiceID = res.values["GROUP(internalid.applyingTransaction)"].value;
