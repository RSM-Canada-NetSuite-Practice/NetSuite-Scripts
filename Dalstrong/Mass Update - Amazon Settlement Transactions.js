...
define(['N/file', 'N/search', 'N/record', 'N/currency'], function(file, search, record, currency) {
   ...
   filters:
   [
      ["custrecord_celigo_amzio_set_amz_account","anyof","102"],
      "AND",
      ["custrecord_celigo_amzio_set_recond_trans","anyof","@NONE@"],
      "AND",
      ["custrecord_celigo_amzio_set_tran_type","noneof","3"]
   ],
   columns:
   [
      ...
      search.createColumn({name: "custrecordrsm_marketplace_cus_tran_settl", label: "Marketplace"}),
      search.createColumn({name: "custrecord_celigo_amzio_set_tran_type", label: "Transaction Type"}),
      search.createColumn({name: "custrecord_celigo_amzio_set_exp_to_io", label: "Exported to IO"}),
